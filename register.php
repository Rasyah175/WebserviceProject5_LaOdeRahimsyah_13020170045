...
if ($conn){
    if (isset($_POST['username']) && isset($_POST['password']) && isset($_POST['nama_lengkap']) && isset($_POST['alamat']) && isset($_POST['kontak'])){
        ...
        $query = $conn->prepare("INSERT INTO account VALUES(:username, :password, :nama_lengkap, :kontak, :alamat, 'default.jpg')");
        ...
        if ($exec) {
            ...
            $data = [
                'status' => FALSE,
                'message' => 'Gagal register, kemungkinan username telah digunakan'
            ];
